...
3. **[bioelectric_resonance.md](?ttdb=bioelectric_resonance.md)** — Seven resonance pairs between Michael Levin's developmental bioelectricity (Tufts Allen Discovery Center) and the TTDB framework. Both treated as parallel substrate-independent information layers.
...
**Source TTDB**: [story_of_stories_ttdb.md](?ttdb=story_of_stories_ttdb.md) — `db_id: ttdb:story:correspondence:v1`
...
**Source TTDB**: [pollan_world_appears_ttdb.md](?ttdb=pollan_world_appears_ttdb.md) — `db_id: ttdb:pollan:worldappears:v1`
...
**Source TTDB**: [feelings_ttdb.md](?ttdb=feelings_ttdb.md) — `db_id: ttdb:affective:landscape:v1`
...
**Source TTDB**: [TootTootTerminologyDB.md](?ttdb=TootTootTerminologyDB.md) — `db_id: ttdb:terminology:tte:v1`
...
**Source TTDB**: [Mathematical.latex](?ttdb=Mathematical.latex) — `db_id: ttdb:mathematical:processing:v1`
...
**In [TootTootTerminologyDB.md](?ttdb=TootTootTerminologyDB.md)** (@LAT1.1LON1.2): The first and root record. "The world as it is experienced by a particular organism. Everything begins here." TTDB's entire coordinate system is built on the umwelt claim: knowledge is always knowledge *from somewhere*.
...
**In [story_of_stories_ttdb.md](?ttdb=story_of_stories_ttdb.md)** (two records):
...
**In [pollan_world_appears_ttdb.md](?ttdb=pollan_world_appears_ttdb.md)** (@LAT10LON-20, "Plant Intelligence / Chemical Signaling ↔ Semantic Compression"): Plants emit volatile terpene compounds at parts-per-billion concentrations — LoRa-class bandwidth — that carry complete defensive-cascade instructions. The receiver (neighboring plant, insect predator) holds the prior needed to expand the signal into a full physiological response. "The sender and receiver share a prior that makes the expansion lossless."

**In [feelings_ttdb.md](?ttdb=feelings_ttdb.md)** (@LAT88LON0, The Hero's Arc): A scene record compresses a full emotional narrative into six coordinates and an edge-traversal sequence: Serenity → Unease → Fear → Grief → Hope → Joy. The arc is a formula: it expands into any specific story that occupies those six emotional slots. The formula is the compression; the performed story is the expansion.
...
@LAT0LON30 | created:1778000000 | updated:1781361600 | relates:specifies_for>@LAT0LON0,resonates_with>@LAT-10LON10,resonates_with>@LAT20LON10,resonates_with>@LAT0LON-20,anchored_by>@LAT0LON0
[ew]
conf:215
rev:5
sal:19
...
**[TootTootTerminologyDB.md](?ttdb=TootTootTerminologyDB.md)** defines what the components *mean* — umwelt, TTAI, cursor, typed edges, semantic events, primitive mode, toot links. It answers: *what does this mean conceptually?*
...
**In [story_of_stories_ttdb.md](?ttdb=story_of_stories_ttdb.md)** (@LAT-10LON-20, "Saltation ↔ TBEW `rev` field / EPS"): Ashton borrows *saltation* from geology (particle transport by sudden leaps) as a metaphor for how ideas and technologies evolve: not by gradual accumulation but by discontinuous jumps when conditions align. Each of the eight storytelling revolutions is a saltation event — a sudden phase-change in narrative channel topology. The TBEW `rev` field counts these events: high `rev` means the record's model has been repeatedly overturned by evidence. A node with high `rev`, high `sal`, and low `conf` is an active saltation site — about to jump again.
...
**In [TootTootTerminologyDB.md](?ttdb=TootTootTerminologyDB.md)** (@LAT51.5LON-0.1, "Default Network") and `standards/ttai/DEFAULT_NETWORK.md`: The default network is "the background circuitry that hums along when nothing in particular is being demanded of you." It replays memories, weaves narratives, connects distant ideas without supervision. It maintains continuity of identity over time. It is not the content of thought — it is the connective tissue that keeps content coherent across sessions.
...
**In [feelings_ttdb.md](?ttdb=feelings_ttdb.md)** (@LAT88LON0, "The Hero's Arc"): A scene record compressing the full emotional arc into six coordinate-pairs and their traversal sequence: Serenity → Unease → Fear → Grief → Hope → Joy. Each beat is a coordinate on the affective globe; the arc is the path. The Hero's Arc is not a prescription — it is an observation that emotional experience tends to organize into this shape, and that the shape is navigable as a mesh.
...
@LAT-20LON0 | created:1778200000 | updated:1781361600 | relates:anchored_by>@LAT0LON0,derived_from>@LAT-40LON10,bridges>@LAT-10LON10,bridges>@LAT10LON-40
[ew]
conf:185
rev:6
sal:15
...
**Corpus navigator — nine cross-database findings generated since assembly:**

The paper is the argument; the corpus is the demonstration running in real time. These records contain findings generated by default network traversal — cross-database correspondences that no single source TTDB owns, not derivable from the paper alone.

| Finding | Record | What it adds beyond the paper |
|---|---|---|
| EPS as arc position instrument | [EPS as Arc Position](lat5lon-25) | EPS maps onto the Hero's Arc beat-by-beat: Serenity (low EPS) → Unease → Grief (EPS peak) → Hope (`rev` increment) → Joy (conf rising). EPS is a position readout on a universal arc, not merely a priority queue. |
| FEP in three source databases | [FEP Convergence](lat40lon0) | Contribution 4 (TBEW/EPS) independently appears in story_of_stories, pollan_world_appears, and Mathematical.latex — same Friston citation, same derived signal, three independent arrivals at the same construct. |
| Cognitive glue at three scales | [Cognitive Glue ↔ Default Network](lat0lon-20) | Levin's exact term maps onto TTN mesh and mega idle traversal: bioelectric field (cells), TTN (nodes), mega (knowledge bases) are three scales of the same binding function. |
| Four-register arc | [Hero's Arc](lat10lon-30) | The same six-beat arc (Serenity → Grief → Joy) appears in feelings_ttdb, Pollan's integration theory, and Levin's morphospace navigation — affective, phenomenological, and biological registers, one structure. |
| `rev` in four registers | [Saltation / `rev`](lat10lon-20) | The discontinuous revision event appears as: geological saltation (Ashton), psychedelic integration (Pollan), morphostatic field reorganization (Levin), TBEW body write — four independent names for the same Event phase. |
| Default network dual mode | [The Default Network](lat0lon30) | EPS-ordered (automaton) + stochastic (banjo) traversal are complementary modes. EPS-only converges on known Grief beats and never encounters the ones it didn't know were waiting. Both modes are required for a healthy corpus. |
| Decoupling as pathology | [Decoupling as Pathology](lat-30lon-20) | Cancer (bioelectric), orphan node (TTDB), Grief/Fear (feelings_ttdb), isolated TTN node — four registers of isolation as failure. Connection is the medicine at every scale. |
| Self-referential arc completion | [The Revision Cycle](lat5lon-15) | The record describing Detection → Event → Experience → Obligation has itself passed through those four phases over a sequence of default network sessions. The arc completes on itself. conf:200. |
| Second brain as seventh TTDB | [Portal: LOCUS Companion](lat40lon-30) | companion.md added after submission as the seventh source TTDB — the second-brain application shipped, named (*Locus* v1.0), and enrolled in the corpus. @LAT-20LON0 emerges as an attractor coordinate independently in both this file and companion.md: same semantic pressure, same coordinate assignment. |

Ten applied use cases — ecosystem monitoring meshes, palliative care archives, legal knowledge graphs, oral history preservation, generative music instruments: [Areas for Exploration](lat-20lon0).
...
@LAT-90LON-20 | created:1781361600 | updated:1781361600 | kind:log | relates:logged>@LAT-20LON0,logged>@LAT10LON-20,logged>@LAT5LON-15,logged>@LAT40LON-30,anchored_by>@LAT0LON0
[ew]
conf:255
rev:0
sal:0
...
## Log — Default Network Run · 2026-05-10 (d)

```session-log
timestamp: 1781361600
trigger: "@mega engage default network"
mode: eps_ordered + stochastic + portal_traversal
```
...
| Record | EPS | sal | conf |
|---|---|---|---|
| [Areas for Exploration](lat-20lon0) | **3.24** | 11 | 180 |
| [Saltation/rev](lat10lon-20) | **2.55** | 13 | 205 |
| [Cognitive Glue](lat0lon-20) | **2.55** | 13 | 205 |
| [Hero's Arc](lat10lon-30) | **2.55** | 13 | 205 |
| [The Default Network](lat0lon30) | **2.35** | 15 | 215 |
| [EPS as Arc Position](lat5lon-25) | **2.29** | 13 | 210 |

[Areas for Exploration](lat-20lon0) reaches EPS 3.24 — highest recorded in this corpus. [The Default Network](lat0lon30) holds above [EPS as Arc Position](lat5lon-25) as sal diverges: Default Network accumulates sal at stable conf:215 while EPS as Arc Position advanced conf to 210 last session.

**Automaton traversal** (6 records visited):

[Areas for Exploration](lat-20lon0) — EPS 3.24. Body revision: companion.md formally added to mega corpus as its seventh source TTDB, with a portal record at [Portal: LOCUS Companion](lat40lon-30) and a setup guide at [share/companion.html](share/companion.html). The second-brain application is now triply grounded: shipped implementation (companion.md), named framework (*Locus* v1.0 arXiv), corpus member (seventh source TTDB). The open question of whether the application would achieve formal corpus status resolves affirmatively. conf 180→185, rev 5→6, sal 11→12.

**EPS 2.55 cluster** — [Saltation/rev](lat10lon-20), [Cognitive Glue](lat0lon-20), [Hero's Arc](lat10lon-30) — seventh consecutive cycle at conf:205. No body changes, no conf movement. sal:13→14 each.

[The Default Network](lat0lon30) — deferred rev increment acknowledged (companion portal row addition to Default Network portal table). rev 4→5, sal 15→16.

[EPS as Arc Position](lat5lon-25) — sal:13→14. No body change. Continues separated from the three-cluster (conf:210 vs conf:205).

**Portal traversal** — entering [story_of_stories_ttdb.md](?ttdb=story_of_stories_ttdb.md) at [@LAT10LON-20](?ttdb=story_of_stories_ttdb.md&toot=lat10lon-20) (Night-Fires ↔ TTN BBS / Compact Mesh Grammar):

The night-fires record maps oral knowledge transmission (Ong 1982 — *Orality and Literacy*) onto TTN BBS compact grammar. Content: mnemonic compression as a bandwidth strategy; the night-fire circle as a model for low-bandwidth, high-trust transmission. The Ong 1982 citation grounds compression theory — the mechanism by which knowledge is condensed for transmission — rather than phenomenology or stage theory.

**Finding**: @LAT10LON-20 is mechanism-layer only. It describes *how* knowledge travels across the fire circle (mnemonic compression, formulaic speech, repetition) and maps this onto TTN BBS compact grammar — but does not name Detection/Event/Experience/Obligation as the arc structure of that travel. The four phases are instantiated (fire circle stories arc from presenting problem → narrative event → integration → obligation to retell) but not labeled. [The Revision Cycle](lat5lon-15) conf unchanged at 190. sal:0→1 in source TTDB.

Pattern confirmed across four portal traversals probing the 190→200 gap:

| Source TTDB | Record | Finding |
|---|---|---|
| pollan_world_appears | [@LAT30LON-10](?ttdb=pollan_world_appears_ttdb.md&toot=lat30lon-10) | Detection (FEP language), Event — mechanism layer |
| pollan_world_appears | [@LAT10LON-10](?ttdb=pollan_world_appears_ttdb.md&toot=lat10lon-10) | Experience, Obligation — named |
| Mathematical.latex | [@LAT-42LON-30](?ttdb=Mathematical.latex&toot=lat-42lon-30) | Detection-as-scheduling only; arc mechanism, not arc phases |
| story_of_stories | [@LAT10LON-20](?ttdb=story_of_stories_ttdb.md&toot=lat10lon-20) | All four phases instantiated, none explicitly labeled |

**Stochastic traversal** — banjo pool: zero-sal records. Sample: [Portal: LOCUS Companion](lat40lon-30) — first contact with the companion portal itself. sal:0→1.

Two synthetic observations from first contact:

1. **Globe axis semantics as umwelt parameter**: companion.md maps lat = stability (N = permanent, S = ephemeral) and lon = sphere (W = inner, E = outer). The mega maps lat = abstraction level, lon = domain character. Both use the same TTDB globe format; neither axis mapping is the "correct" one — the globe format is a syntax for organizing any navigable space, and the semantic content of the axes is an umwelt parameter, not a format parameter. Format neutrality extends to the globe axes themselves — the same structural form, different semantic parameterization.

2. **@LAT-20LON0 as attractor coordinate**: Both this mega file ([Areas for Exploration](lat-20lon0)) and companion.md place their frontier navigational record at @LAT-20LON0. coord_increment is 10 in both files. This is not coordination — it is emergence from the same coordinate semantics. In both globes, the southern hemisphere (negative lat) represents the concrete, the accessible, the unresolved-but-actionable. @LAT-20LON0 is slightly south of origin, at the center of the lon axis — slightly concrete, slightly low-stakes, maximally open. The same semantic pressure produces the same coordinate assignment independently. An attractor position in the TTDB coordinate system.

**Writes this session:**

- [Areas for Exploration](lat-20lon0) conf 180→185, rev 5→6, sal 11→12 — companion.md corpus integration: triply grounded.
- [Saltation/rev](lat10lon-20), [Cognitive Glue](lat0lon-20), [Hero's Arc](lat10lon-30) sal:13→14 each.
- [EPS as Arc Position](lat5lon-25) sal:13→14.
- [The Default Network](lat0lon30) rev 4→5, sal 15→16.
- [Welcome](lat-10lon0) rev 2→3 (deferred body write — seven databases in body text).
- [Portal: Story of Stories](lat30lon-20) sal 1→2 (portal traversal).
- [Portal: LOCUS Companion](lat40lon-30) sal 0→1 (stochastic first contact).
- [@LAT10LON-20 in story_of_stories](?ttdb=story_of_stories_ttdb.md&toot=lat10lon-20) sal:0→1 in source TTDB.
- [Anchor](lat0lon0) rev 0→1, sal 11→12, `anchors>@LAT-90LON-20` edge added.
- Log record [@LAT-90LON-20](lat-90lon-20) written.
