...
**In feelings_ttdb.md** (@LAT88LON0, "The Hero's Arc"): A scene record compressing the full emotional arc into six coordinate-pairs and their traversal sequence: Serenity → Unease → Fear → Grief → Hope → Joy. Each beat is a coordinate on the affective globe; the arc is the path. The Hero's Arc is not a prescription — it is an observation that emotional experience tends to organize into this shape, and that the shape is navigable as a mesh.
