...
**One file. One AI. Yours. Rooted in the Locus framework.**

`companion.md` is a personal agent companion that lives entirely in a single Markdown file and is grounded in the Locus framework for experiential modeling. Drop it in a folder, open it alongside a conversation with Claude (or any capable AI), and you have a companion that carries context across every session — because the file *is* the memory.

---

## What it does

LOCUS maintains a structured record of who you are, what you're working toward, and what you haven't figured out yet. Between conversations, it runs a background process — the *default network* — that scans for records that need attention, flags stale goals, and notices connections you haven't made explicit yet. Built on the Locus framework, it models your development as transitions: moving from perceived states to new states, understanding deepening over time.
...
Pick a name. Open the file and replace every instance of `LOCUS` with it. One find-and-replace, done. The name becomes your invocation prefix: `@EMBER`, `@ATLAS`, `@REX`, whatever fits.
...
```
@LOCUS fill in Your Profile together
```

LOCUS will ask you the questions in the Profile record and help you write the answers. Once that record is complete, every future conversation starts with full context.

---

## How to use it

Prefix any message with `@LOCUS` to invoke the companion:

```
@LOCUS what should I focus on today?
@LOCUS I just finished the landing page — mark that goal done
@LOCUS I keep avoiding the budget conversation. Add that to Open Questions.
@LOCUS STATUS
@LOCUS LOG noticed I'm more productive in the morning than I thought
```
...
|---|---|
| `@LOCUS STATUS` | Shows EPS rankings — which records need attention — and any stale or flagged items |
| `@LOCUS LOG <note>` | Appends a brief observation to the active log record |
| `@LOCUS FOCUS <record>` | Moves the cursor to a specific record and marks it as active |
| `@LOCUS FIND <word>` | Searches records for a term |
| `@LOCUS SELECT <id>` | Navigates directly to a record by coordinate, e.g. `SELECT lat20lon0` |

---

## How memory works

Every piece of context in `companion.md` is a **record** — a named section with a coordinate address (`@LATxLONy`) and a small metadata block called `[ew]` (epistemic weight). LOCUS is grounded in the Locus framework, which models memory transitions: how understanding evolves from "I've noted this" to "I understand this."
...
From these, LOCUS derives **EPS = sal × (255 − conf) / 255** — the *epistemic pressure signal*. A record with high EPS has been consulted often but remains poorly understood. That's a record asking to be revisited.
...
**This file is your umwelt.** Jakob von Uexküll's *umwelt* is the species-specific perceptual slice each organism inhabits — not the world, but the world as it registers to a particular perceiving subject. Your companion file is exactly this: a bounded, personal record of what is *sign-worthy* to you. The Locus framework grounds this in formal perceptual transitions: `@PERCEPT:before` → `@PERCEPT:after`, meaning you move from one state of understanding to another. Not a comprehensive catalog. *Experiential sufficiency*.

This is why incompleteness is a feature, not a bug. A record with low `conf` is one where the model hasn't been built yet. A record with high EPS is one where the transition from *"I've noted this"* to *"I understand this"* hasn't happened yet. LOCUS's job is to help you complete those transitions — not to fill every field.
...
|---|---|---|
| LOCUS (anchor) | `@LAT0LON0` | Identity root — who LOCUS is and how memory works |
...
| Default Network | `@LAT0LON20` | What LOCUS does between conversations |
...
**Rename the companion**: find-and-replace `LOCUS` → your chosen name throughout the file. The invocation prefix in the `mmpdb` block updates automatically.

**Fill in Your Profile**: this is the most important step. The `conf:64` on that record is a signal that it's incomplete — raise it toward `200` as LOCUS's responses start to consistently reflect who you are.
...
3. Write what happened, what LOCUS noticed, and what records changed
4. Link to changed records using toot links: `[record name](latXlonY)`

The log is LOCUS's audit trail. It is how you know what changed and when.
...
`companion.md` implements the **TTDB** (Toot-Toot Database) file format grounded in the **Locus** framework — the same format used by the A32 autonomous ESP32 agent framework, the Toot Toot mesh network node logger, and the A32 Mega multi-corpus librarian. A companion you build here can be compiled into a deployment TTDB for a microcontroller, federated across a TTN mesh node, or extended into a multi-corpus mega instance.
